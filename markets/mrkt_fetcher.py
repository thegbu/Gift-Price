...
async def get_token(session: aiohttp.ClientSession, init_data: str) -> str | None:
    """Fetches auth token from MRKT API using a provided session."""
    try:
...
async def get_mrkt_prices(collection_name: str, model_name: str, backdrop_name: str) -> tuple[int | None, int | None]:
